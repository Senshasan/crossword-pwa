...
export default function Page() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      if (data.user) {
        ...
        const { data: puzzleRows } = await supabase
          .from("puzzles")
          .select("id,title,puzzle_type,order_index,width,height,grid,clues")
          .order("order_index");
        if (puzzleRows)
          setPuzzles(
            (puzzleRows as PuzzleRow[])
              .map(mapPuzzle)
    ...
    const { data: listener } = supabase.auth.onAuthStateChange(
      (_event, session) => {
        ...
        if (!session) setProfileReady(false);
      },
    );
    return () => {
      mounted = false;
  ...
  const nextPuzzle =
    puzzles.find((p) => !completed.includes(p.id)) ?? puzzles[0];
  const activeClue = findActiveClue(active, cursor, direction);
  const wordCells = activeClue ? getWordCells(active, activeClue) : [];
  const isSolved = allCells(active).every(
    (cell) => answers[keyFor(cell.row, cell.col)] === cell.letter,
  );
  ...
  function handleKey(event: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (event.key === "Backspace") {
      ...
      if (answers[current]) setAnswers({ ...answers, [current]: "" });
      else {
  ...
  function checkWord() {
    ...
    wordCells.forEach((cell) => {
      ...
      if (
        next[keyFor(cell.row, cell.col)] &&
        next[keyFor(cell.row, cell.col)] !== expected
      )
        next[keyFor(cell.row, cell.col)] = "";
    });
    setAnswers(next);
  ...
  function checkGrid() {
    ...
    allCells(active).forEach((cell) => {
      if (
        next[keyFor(cell.row, cell.col)] &&
        next[keyFor(cell.row, cell.col)] !== cell.letter
      )
        next[keyFor(cell.row, cell.col)] = "";
    });
    setAnswers(next);
...
function PlayScreen({
  ...
  const wordSet = new Set(wordCells.map((c: any) => keyFor(c.row, c.col)));
  return (
    <main className="min-h-screen bg-background text-foreground">
      <header className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5 sm:px-8">
      ...
      <section className="mx-auto max-w-6xl px-5 pb-12 sm:px-8">
        ...
        <div className="play-layout mt-8">
          <div className="grid-wrap">
            ...
            >
              {active.grid.map((row: any[], r: number) =>
                row.map((cell: any, c: number) => {
                  const key = keyFor(r, c);
                  return (
                    <button
                      key={key}
                      className={`cell ${cell.isBlocked ? "blocked" : ""} ${wordSet.has(key) ? "word-active" : ""} ${cursor.row === r && cursor.col === c ? "selected" : ""}`}
                      onClick={(event) => {
